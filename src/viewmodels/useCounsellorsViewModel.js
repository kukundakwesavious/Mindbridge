...
export function useCounsellorsViewModel() {
  ...
  const filtered = useMemo(() => counsellors.filter(c => {
    const matchesFilter = filter === 'All' || (filter === 'Available' ? c.available : c.languages.includes(filter));
    const q = query.trim().toLowerCase();
    const matchesQuery = !q || `${c.name} ${c.role} ${c.speciality}`.toLowerCase().includes(q);
